...
const WrappedRetroEffect = wrapEffect(RetroEffectImpl) as unknown as React.ComponentType<{
  colorNum: number;
  pixelSize: number;
...
const RetroEffect = forwardRef<RetroEffectImpl, { colorNum: number; pixelSize: number }>((props, ref) => (
  <WrappedRetroEffect ref={ref} colorNum={props.colorNum} pixelSize={props.pixelSize} />
));
...
function DitheredWaves({
  ...
  useFrame(({ clock }) => {
    ...
    if (enableMouseInteraction) {
      (u.mousePos as THREE.Uniform<THREE.Vector2>).value.copy(mouseRef.current);
    }
  });
